...
client.once("ready", async () => {
    ...
    let allWallsOfText = [
        ...
        "./media/1644871169687.mp4",
        "./media/epicemoji.png",
        "./media/geyshinimpact.png",
        "./media/f77453938f8f65071d2a448b68f3af23.mp4",
        ...
        "./media/skype_-_Discord_2021-07-02_00-54-23.mp4",
    ...
    function dailyWallOfText() {
        ...
        if(!(randomWallsOfText[rng].startsWith("./"))) {
            channel.send(randomWallsOfText[rng]);
        } else {
            channel.send({files: [{attachment: randomWallsOfText[rng]}]});
        }
    }
...
client.on("messageCreate", async message => {
    ...
    if (message.mentions.users.has(config.clientId) && !message.author.bot && !message.content.startsWith(prefix)) {
        ...
        let randomResponses = [
            ...
            "Знаєш, ти блять собака йонаа нахуй, ти народився на цей світ потслучайності, ти помилка людства блять",
            "Уйобки піздєц, спасіба блять.",
            "Слухай ти блять чмо блять",
            "Завали єбало блять",
            "Ти звідки виліз нахуй",
            "Пішов нахуй агресор йобаний",
            "Їбать ти нахуй злий впизду даун серлишся блять нахуц сука обколовся їбать героїном нахуй в бзш22 купив вицобуєшся бульбулятором за школой курив блять нахуй я вахуе їбануться їбаться кусаться їбанись колонись єбансь",
            "You put your fat dick between their ass",
            "Як щодо не їбати мені мозг?",
            "БЛЯ то САМЕ ЧУСТВО коли ти блять УЄБАН НАХУЙ ХАХАХХАХАХАХХАХХАХАХХАХАХА",
